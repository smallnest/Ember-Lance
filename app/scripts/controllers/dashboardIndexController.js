...
App.DashboardIndexController = Ember.ObjectController.extend({
    ...
    memory_commit: 1024,
    thread_count: 72,
    thread_peak_count: 100,
    thread_daemon_count: 40,
    cpu_usage: 30,
    disk_usage: 120,
    disk_log_usage: 40,
    disk_mongo_usage: 25,

    requestRateActionButtons: [
        Ember.Object.create({title: 'Submit', clicked:'submitRequestRateAction'}),
        Ember.Object.create({title: 'Cancel', clicked:'cancelRequestRateAction', dismiss: 'modal'})
    ],
    ...
    refreshData: function () {
        this.set("memory_usage", 50 + Math.floor((Math.random() * 200)));
        this.set("memory_usage", 50 + Math.floor((Math.random() * 200)));
        this.set("thread_count", 50 + Math.floor((Math.random() * 50)));
        this.set("thread_peak_count", this.get("thread_peak_count") + Math.floor((Math.random() * 3)));
        this.set("thread_daemon_count", 50 + Math.floor((Math.random() * 2)));
        this.set("cpu_usage", 30 + Math.floor((Math.random() * 70)));
        this.set("disk_usage", 120 + Math.floor((Math.random() * 100)));
        this.set("disk_log_usage", 40 + Math.floor((Math.random() * 10)));
        this.set("disk_mongo_usage", 25 + Math.floor((Math.random() * 20)));
    ...
    showBar: function () {
        ...
        function exampleData() {
            return stream_layers(3,10+Math.random()*20,0.1).map(function(data, i) {
                return {
                    key: 'Stream #' + i,
                    values: data
                };
    ...
    showPie: function () {
        ...
        nv.addGraph(function() {
            ...
            d3.select("#donut-chart").append("svg")
                    .attr("width", "90%")
                    .attr("height", "360px")
                    .datum(exampleData())
                    .transition().duration(350)
    ...
    showRealTime: function () {
        var n = 100,
                ...
                count = 0,
                data = d3.range(n).map(d3.random.normal(50, 20));
        ...
        var line = d3.svg.line()
                .interpolate("basis")
                .x(function(d, i) { return x(now - (n - i) * duration); })
                .y(function(d, i) { return y(d); });

        var svg = d3.select("#realtimeSVG").append("svg")
        ...
        function tick() {
            // update the domains
            now = new Date();
            data.push(d3.random.normal(50, 20).call());
